...
| EP02 | Laboratory Inventory Management | **Como usuario, quiero** registrar, controlar y actualizar mi inventario mediante tecnologías IoT/RFID, y coordinar transferencias de uso **para** garantizar trazabilidad. |
...
| EP04 | Reports and Laboratory Analysis | **Como administrador, quiero** generar reportes de recursos y análisis por laboratorio **para** evaluar el uso y disponibilidad de insumos. |
| EP05 | Alerts and Notifications Manager | **Como administrador, quiero** configurar y recibir alertas críticas y notificaciones automáticas relacionadas al inventario **para** supervisar condiciones y estados del laboratorio que permitan respuestas inmediatas. |
| EP06 | Laboratory Management | **Como inspector o coordinador, quiero** seleccionar y administrar laboratorios, puestos o ubicaciones, así como sus responsables y accesos, **para** garantizar la trazabilidad y seguridad operativa. |
## User Stories
| User Story ID | Título               | Descripción                            | Criterios de aceptación                  | Épica                 |
|---------------|----------------------|--------------------------------------|-----------------------------------------|-----------------------|
# LabIoT Platform – Épicas y User Stories

Las siguientes user stories definen el alcance inicial de la plataforma de gestión inteligente de inventarios para laboratorios, considerando a nuestros tres tipos de usuarios clave:

- **Usuario (investigador líder / responsable de laboratorio)**
- **Administrador (supervisor comercial / jefe de compras)**
- **Inspector (coordinador de cumplimiento / auditor)**
...
| EP03 | Subscription Management | Como usuario, quiero gestionar el proceso completo de suscripción, desde la selección y pago, hasta la activación, prueba y cancelación para acceder a servicios de forma flexible. |
| EP04 | Reports and Laboratory Analysis | Como administrador, quiero generar reportes de recursos y análisis por laboratorio para evaluar el uso y disponibilidad de insumos. |
| EP05 | Alerts and Notifications Manager | Como administrador, quiero configurar y recibir alertas críticas y notificaciones automáticas relacionadas al inventario. |
| EP06 | Laboratory Management | Como inspector o coordinador, quiero seleccionar y administrar laboratorios, puestos o ubicaciones, así como sus responsables y accesos, para garantizar la trazabilidad y seguridad operativa. |

## User Stories

| Código | Título | Descripción | Criterios de aceptación | Épica |
| --- | --- | --- | --- | --- |
| US-01 | Pestaña de Perfil | Como usuario, quiero una pestaña de perfil donde ingresar mis datos personales y profesionales para gestionarlos fácilmente. | Escenario 1: **Given** el usuario desea consultar su perfil, **when** accede a la pestaña "Mi Perfil", **then** puede ver los datos registrados y el botón "Crear Nuevo Perfil" si todavía no existe información.<br>Escenario 2: **Given** el usuario ya registró su información, **when** abre la pestaña nuevamente, **then** visualiza sus datos actuales listos para editarse. | EP01 |
| US-02 | Crear Perfil | Como usuario, quiero crear un perfil completando mi información personal para que otros sepan quién opera el laboratorio. | Escenario 1: **Given** el usuario no tiene perfil, **when** selecciona "Crear Nuevo Perfil" y completa los campos obligatorios, **then** el sistema confirma la creación y muestra el perfil en la pestaña.<br>Escenario 2: **Given** la información está incompleta, **when** intenta guardar, **then** recibe un mensaje indicando qué campos debe corregir antes de continuar. | EP01 |
| US-03 | Modificar Perfil | Como usuario, quiero modificar la información registrada en mi perfil cuando cambien mis datos. | Escenario 1: **Given** el usuario actualiza su número de contacto o especialidad, **when** edita y guarda su perfil, **then** el sistema refleja la modificación y confirma la actualización.<br>Escenario 2: **Given** el usuario agrega nuevos datos pero decide cancelarlos, **when** sale sin guardar, **then** el sistema mantiene la información anterior. | EP01 |
| US-04 | Cargar documentos | Como usuario, quiero cargar documentos relevantes (certificaciones, manuales) dentro de mi perfil para tenerlos accesibles. | Escenario 1: **Given** el usuario tiene un archivo válido, **when** lo adjunta en la sección de documentos, **then** el sistema verifica el formato/tamaño permitido y confirma la carga.<br>Escenario 2: **Given** el archivo no cumple con los parámetros, **when** intenta subirlo, **then** recibe un mensaje explicando por qué no se aceptó. | EP01 |
| US-05 | Registrar el rem con QR/RFID | Como usuario, quiero registrar el Rem en el inventario usando QR/RFID para agilizar el ingreso y evitar errores manuales. | Escenario 1: **Given** el usuario escanea un código válido, **when** completa la guía con datos del Rem, **then** el sistema genera la entrada en inventario con sello de tiempo.<br>Escenario 2: **Given** el QR/RFID no coincide con un lote registrado, **when** intenta finalizar, **then** recibe una alerta para revisar o registrar primero el código maestro. | EP02 |
| US-06 | Actualizar stock automáticamente tras venta/retorno | Como usuario, quiero que el sistema actualice automáticamente el stock después de una salida, retorno o transferencia para contar siempre con datos precisos. | Escenario 1: **Given** un insumo fue retirado y escaneado para salida, **when** se completa la operación, **then** el stock disminuye y se crea un registro en el historial.<br>Escenario 2: **Given** el insumo regresa al laboratorio, **when** se escanea como retorno, **then** el stock aumenta y la trazabilidad refleja la devolución. | EP02 |
| US-07 | Generar historial de uso por insumo | Como usuario, quiero que el sistema genere y muestre un historial de uso detallado para cada insumo, para poder seguir quién lo tiene y qué ocurrió. | Escenario 1: **Given** un insumo tiene movimientos, **when** consulto su historial, **then** veo las acciones (ingreso, préstamo, retorno, baja) con fechas y responsables.<br>Escenario 2: **Given** un insumo no tiene eventos, **when** abro su historial, **then** el sistema indica que aún no hay registros. | EP02 |
| US-08 | Selección de plan de suscripción | Como usuario, quiero poder elegir entre distintos planes de suscripción según las necesidades de mi laboratorio. | Escenario 1: **Given** el usuario evalúa planes, **when** revisa la tabla comparativa, **then** puede seleccionar uno y avanzar al formulario de pago.<br>Escenario 2: **Given** el plan incluye periodo de prueba, **when** confirma la suscripción, **then** se indica la fecha de inicio y la duración del trial. | EP03 |
| US-09 | Registrar método de pago | Como usuario, quiero guardar mi método de pago para agilizar renovaciones futuras. | Escenario 1: **Given** el usuario ingresa los datos de su tarjeta o medio autorizado, **when** confirma, **then** el sistema valida la información y la asocia al plan activo.<br>Escenario 2: **Given** se detecta un dato inválido, **when** intenta registrar el método, **then** recibe un aviso para corregirlo. | EP03 |
| US-10 | Activar periodo de prueba | Como usuario, quiero activar un periodo de prueba antes de comprometerme a una suscripción paga. | Escenario 1: **Given** el plan ofrece prueba, **when** el usuario la habilita, **then** recibe confirmación con fecha de fin y condiciones.<br>Escenario 2: **Given** la prueba está por terminar, **when** recibe una notificación, **then** puede decidir si continuar con el plan o cancelarlo sin penalidad. | EP03 |
| US-11 | Cancelar suscripción | Como usuario, quiero cancelar mi suscripción en cualquier momento para gestionar mejor mi presupuesto. | Escenario 1: **Given** el usuario ya no necesita el servicio, **when** solicita la cancelación, **then** el sistema confirma la fecha efectiva y bloquea nuevos cargos.<br>Escenario 2: **Given** la cancelación se solicita en periodo de prueba, **when** se procesa, **then** se notifica que no habrá cobros posteriores. | EP03 |
| US-12 | Recibir alerta de temperatura | Como administrador quiero recibir una alerta cuando un equipo supere la temperatura permitida, para evitar daños a los insumos. | Escenario 1: **Given** se detecta que un sensor supera el límite, **when** ocurre el evento, **then** el sistema envía una notificación (correo/app/SMS) al administrador.<br>Escenario 2: **Given** el administrador recibe la alerta, **when** accede al panel, **then** puede ver el historial del sensor y el insumo afectado. | EP05 |
| US-13 | Recibir alerta de insumos próximos a vencer | Como administrador quiero recibir alertas sobre insumos próximos a vencer para planificar su uso o reemplazo. | Escenario 1: **Given** un insumo está a 30 días de su caducidad, **when** el sistema procesa la fecha, **then** se envía una notificación con recomendaciones.<br>Escenario 2: **Given** se agotó el tiempo de reacción, **when** llega la fecha límite, **then** se genera una alerta crítica indicando que el insumo debe retirarse. | EP05 |
| US-14 | Recibir alerta de bajo stock de insumos | Como administrador quiero recibir una alerta cuando el inventario llegue al mínimo definido para reponer material a tiempo. | Escenario 1: **Given** un ítem alcanza el mínimo, **when** se registra la salida que lo provoca, **then** se envía la alerta de bajo stock con sugerencia de compra.<br>Escenario 2: **Given** se repone el inventario, **when** se registra la entrada, **then** la alerta se marca como atendida. | EP05 |
| US-15 | Configurar tipos y canales de alertas | Como administrador quiero configurar el tipo de alertas y los canales (correo, SMS, app) para adaptar las notificaciones al tipo de insumo. | Escenario 1: **Given** ingresa a la sección de alertas, **when** selecciona los canales y umbrales, **then** el sistema guarda la configuración por laboratorio.<br>Escenario 2: **Given** necesita pausar un canal, **when** lo desactiva temporalmente, **then** las alertas dejan de enviarse por ese medio hasta reactivarlo. | EP05 |
| US-16 | Generar reporte de inventario | Como administrador quiero generar un reporte semanal del inventario, incluyendo movimientos, ajustes y alertas. | Escenario 1: **Given** selecciona rango de fechas y laboratorio, **when** genera el reporte, **then** obtiene un documento con saldos iniciales/finales, movimientos y responsables.<br>Escenario 2: **Given** necesita compartirlo, **when** exporta en PDF o CSV, **then** el archivo se descarga con el mismo contenido. | EP04 |
| US-17 | Dashboard de métricas de uso | Como administrador quiero visualizar un dashboard con estadísticas de insumos utilizados, prácticas realizadas y alertas para analizar tendencias. | Escenario 1: **Given** el administrador ingresa al dashboard, **when** consulta el gráfico de insumos consumidos por mes, **then** puede ver la tendencia en porcentaje.<br>Escenario 2: **Given** desea comparar laboratorios, **when** aplica filtros, **then** el dashboard actualiza los indicadores en tiempo real. | EP04 |
| US-18 | Reporte de recursos críticos | Como administrador quiero consolidar los insumos críticos por laboratorio y responsable para priorizar compras y distribución. | Escenario 1: **Given** selecciona "Reporte de recursos críticos", **when** ejecuta la consulta, **then** obtiene la lista con cantidad disponible, estado y responsable.<br>Escenario 2: **Given** la lista se exporta, **when** descarga el archivo, **then** puede compartirlo con los responsables involucrados. | EP04 |
| US-19 | Exportar reportes automatizados | Como administrador quiero programar la exportación automática de reportes para enviarlos periódicamente a los stakeholders. | Escenario 1: **Given** define una frecuencia y destinatarios, **when** guarda la programación, **then** el sistema envía el reporte según la agenda.<br>Escenario 2: **Given** necesita pausar un envío, **when** desactiva la programación, **then** se detiene hasta que vuelva a activarla. | EP04 |
| US-20 | Seleccionar laboratorio | Como inspector quiero poder seleccionar un laboratorio o puesto específico para revisar su operación. | Escenario 1: **Given** el inspector ingresa a la plataforma, **when** abre el selector de laboratorios, **then** puede elegir uno y acceder a su información.<br>Escenario 2: **Given** requiere comparar puestos dentro del mismo laboratorio, **when** usa los filtros por ubicación, **then** obtiene la vista correspondiente. | EP06 |
| US-21 | Asignar responsables | Como inspector o coordinador quiero asignar responsables y roles dentro de cada laboratorio para mantener control y trazabilidad. | Escenario 1: **Given** selecciona un laboratorio, **when** asigna un responsable principal y colaboradores, **then** el sistema registra la relación y muestra los roles.<br>Escenario 2: **Given** se reemplaza un responsable, **when** actualiza la asignación, **then** el historial registra el cambio y la nueva persona obtiene acceso. | EP06 |
| US-22 | Subir producto | Como inspector quiero subir un registro de producto, lote o equipo durante una visita para dejar constancia de su estado. | Escenario 1: **Given** el inspector inspecciona un laboratorio, **when** captura datos del producto (etiqueta, estado, fotos), **then** se crea el registro asociado al laboratorio visitado.<br>Escenario 2: **Given** el producto requiere seguimiento, **when** marca la observación como pendiente, **then** el responsable recibe la tarea correspondiente. | EP06 |
